            ...
            <div className="kickback-banner-content" style={{ position: 'relative', zIndex: 1, width: '100%', padding: 'clamp(2rem,5vw,4rem) clamp(1.5rem,6vw,6rem)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '2rem', flexWrap: 'wrap' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '1rem' }}>
                  <span style={{ fontSize: '0.65rem', fontWeight: 800, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--accent)' }}>Kickback · {nextMeet ? 'Next Meet' : 'Community Meets'}</span>
                </div>
                ...
                }}>{nextMeet ? nextMeet.title : 'Car Meets. Community Drives. Good Vibes.'}</h2>
                  ...
                  {nextMeet ? (
                    <>
                      <span style={{ color: 'rgba(240,245,236,0.55)', fontSize: '0.88rem', display: 'flex', alignItems: 'center', gap: 6 }}>
                        📅 {(() => { const [y,m,d] = nextMeet.date.slice(0,10).split('-').map(Number); return new Date(y,m-1,d).toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' }) })()}
                      </span>
                      <span style={{ color: 'rgba(240,245,236,0.55)', fontSize: '0.88rem', display: 'flex', alignItems: 'center', gap: 6 }}>
                        📍 {nextMeet.location}
                      </span>
                    ...
                    <span style={{ color: 'rgba(240,245,236,0.55)', fontSize: '0.88rem' }}>
                      Show up, hang out, talk cars. Hosted by driversCraft.
                    </span>
                  )}
                </div>
                ...
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: 'var(--green-brand)', color: '#fff', padding: '0.75rem 1.5rem', borderRadius: 8, fontWeight: 700, fontSize: '0.9rem' }}>
                  {nextMeet ? 'View →' : 'See All Meets →'}
                </div>
              </div>

              {/* Countdown pill — only when meet exists */}
              {nextMeet && (
                <div style={{
